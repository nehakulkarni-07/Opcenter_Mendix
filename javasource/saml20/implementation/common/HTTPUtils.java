...
public class HTTPUtils {
    ...
    public static String[] extractResourceArguments(IMxRuntimeRequest request) {
        ...
        switch (resourceArgs.length) {
            ...
            case 2:
                ...
                break;
            default:
                if(Constants.SAMLAction.SubmitLoginForm.name().equals(resourceArgs[2])){
                    resourceArgs = new String[]{Constants.SAMLAction.SubmitLoginForm.name(), ""};
                }else{
    ...
    public static void SubmitLoginForm(IMxRuntimeResponse mxResponse) throws IOException {
        mxResponse.setContentType("text/javascript");
        mxResponse.setStatus(HttpServletResponse.SC_OK);
        try(Writer writer = mxResponse.getWriter()) {
            ;
            writer.write("window.onload = (event) => {  document.forms[0].submit(); };");
            writer.flush();
        }
