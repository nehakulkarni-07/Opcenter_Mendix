...
import saml20.proxies.Enum_AssertionConsumerServiceIndex;
...
public class MxSAMLAuthnRequest extends MxSAMLRequest {
    ...
    public static MxSAMLAuthnRequest buildAuthnRequestObject(SAMLRequestContext context, Metadata metadata, org.opensaml.saml.saml2.metadata.Endpoint signonLocation, String relayState, boolean enableForceAuthentication, boolean isInSessionLogin) throws SAMLException {
        // Issuer object
        ...
        SSOConfiguration ssoConfig = SSOConfiguration.initialize(context.getIContext(), metadata.getSsoConfiguration());
        int attributeConsumingServiceIndex = OpenSAMLUtils.getAttributeConsumingServiceIndex(context, metadata, isInSessionLogin);
        if (attributeConsumingServiceIndex >= 0) {
            authRequest.setAttributeConsumingServiceIndex(attributeConsumingServiceIndex);
        }
        Integer index = ssoConfig.getAssertionConsumerServiceIndex();
        if (Enum_AssertionConsumerServiceIndex.Yes.equals(ssoConfig.getEnableAssertionConsumerServiceIndex())) {
        ...
        // These changes are introduced as part of InSession Authentication
        authRequest.setForceAuthn(enableForceAuthentication);
